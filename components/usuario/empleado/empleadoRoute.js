const { addSchema, editSchema, getSchema } = require('./empleadoSchema');
...
router.post('/',
  ...
  async (req, res, next) => {
    await controller.add(req.body).then((data) => {
      addBitacora(req.headers.authorization.split(' ')[1], 'Empleado', data.id, 'Agregó un nuevo empleado');
...
router.put('/:id',
  ...
  async (req, res, next) => {
    ...
    await controller.edit(body, id)
      .then((data) => {
        addBitacora(req.headers.authorization.split(' ')[1], 'Empleado', data.id, 'Editó un empleado');
        response.success(req, res, data, 201);
      }).catch((err) => {
        next(err);
      });
...
router.delete('/:id',
  ...
  async (req, res, next) => {
    ...
    await controller.delete(id)
      .then((data) => {
        addBitacora(req.headers.authorization.split(' ')[1], 'Empleado', id, 'Eliminó un empleado');
        response.success(req, res, data, 201);
      }).catch((err) => {
        next(err);
      });

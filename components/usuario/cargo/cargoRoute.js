const { addSchema, editSchema, getSchema } = require('./cargoSchema');
...
router.post('/',
  ...
  async (req, res, next) => {
    await controller.add(req.body).then((data) => {
      addBitacora(req.headers.authorization.split(' ')[1], 'Cargo', data.id, 'Agregó un nuevo cargo');
...
router.put('/:id',
  ...
  async (req, res, next) => {
    ...
    await controller.edit(body, id)
      .then((data) => {
        addBitacora(req.headers.authorization.split(' ')[1], 'Cargo', data.id, 'Editó un cargo');
        response.success(req, res, data, 201);
      }).catch((err) => {
        next(err);
      });
...
router.delete('/:id',
  ...
  async (req, res, next) => {
    ...
    await controller.delete(id)
      .then((data) => {
        addBitacora(req.headers.authorization.split(' ')[1], 'Cargo', id, 'Eliminó un cargo');
        response.success(req, res, data, 201);
      }).catch((err) => {
        next(err);
      });

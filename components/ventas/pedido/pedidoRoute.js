const { addSchema, editSchema, getSchema } = require('./pedidoSchema');
...
router.post('/',
  ...
  async (req, res, next) => {
    await controller.add(req.body).then((data) => {
      addBitacora(req.headers.authorization.split(' ')[1], 'Pedido', data.id, 'Agregó un nuevo pedido');
...
router.put('/:id',
  ...
  async (req, res, next) => {
    ...
    await controller.edit(body, id)
      .then((data) => {
        addBitacora(req.headers.authorization.split(' ')[1], 'Pedido', id, 'Editó un pedido');
        response.success(req, res, data, 201);
      }).catch((err) => {
        next(err);
      });
...
router.delete('/:id',
  ...
  async (req, res, next) => {
    ...
    await controller.delete(id)
      .then((data) => {
        addBitacora(req.headers.authorization.split(' ')[1], 'Pedido', id, 'Eliminó un pedido');
        response.success(req, res, data, 201);
      }).catch((err) => {
        next(err);
      });

const { addSchema, editSchema, getSchema } = require('./compraSchema');
...
router.get('/',
  ...
  checkRoles('administrador', 'inventario', 'inventario'),
  async (req, res, next) => {
...
router.post('/',
  ...
  async (req, res, next) => {
    await controller.add(req.body).then((data) => {
      addBitacora(req.headers.authorization.split(' ')[1], 'Compra', data.id, 'Agregó una nueva compra');
...
router.put('/:id',
  ...
  async (req, res, next) => {
    ...
    await controller.edit(body, id)
      .then((data) => {
        addBitacora(req.headers.authorization.split(' ')[1], 'Compra', id, 'Editó una compra');
        response.success(req, res, data, 201);
      }).catch((err) => {
        next(err);
      });
...
router.delete('/:id',
  ...
  async (req, res, next) => {
    ...
    await controller.delete(id)
      .then((data) => {
        addBitacora(req.headers.authorization.split(' ')[1], 'Compra', id, 'Eliminó una compra');
        response.success(req, res, data, 201);
      }).catch((err) => {
        next(err);
      });

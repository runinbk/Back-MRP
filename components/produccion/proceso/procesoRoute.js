const { addSchema, editSchema, getSchema } = require('./procesoSchema');
const { checkRoles } = require('../../../middleware/roleHandler');
const validatorHandler = require('../../../middleware/validatorHandler');
const response = require('../../../network/response');
const procesoController = require('./procesoController');
const express = require('express');
const passport = require('passport');
const addBitacora = require('../../../libs/bitacora');

const router = express.Router();
const controller = new procesoController();
...
router.post('/',
  ...
  async (req, res, next) => {
    await controller.add(req.body).then((data) => {
      addBitacora(req.headers.authorization.split(' ')[1], 'Proceso', data.id, 'Agregó un proceso');
...
router.put('/:id',
  ...
  async (req, res, next) => {
    ...
    await controller.edit(body, id)
      .then((data) => {
        addBitacora(req.headers.authorization.split(' ')[1], 'Proceso', id, 'Editó un proceso');
        response.success(req, res, data, 201);
      }).catch((err) => {
        next(err);
      });
...
router.delete('/:id',
  ...
  async (req, res, next) => {
    ...
    await controller.delete(id)
      .then((data) => {
        addBitacora(req.headers.authorization.split(' ')[1], 'Proceso', id, 'Eliminó un proceso');
        response.success(req, res, data, 201);
      }).catch((err) => {
        next(err);
      });
